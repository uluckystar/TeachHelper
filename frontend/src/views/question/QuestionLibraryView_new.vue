...
const loadSubjects = async () => {
  try {
    const subjectsData = await subjectApi.getAllSubjectNames()
...
const loadGradeLevels = async () => {
  try {
    const gradeLevelsData = await gradeLevelApi.getAllGradeLevelNames()
    gradeLevels.value = gradeLevelsData || []
  } catch (error) {
...
const getSourceTypeName = (sourceType: string, question: any) => {
  const map: Record<string, string> = {
    'SELF_CREATED': '自创',
    'INTERNET': '互联网',
    'AI_GENERATED': 'AI生成',
  ...
  let name = map[sourceType] || '未知'
  
  // 如果是题目库或知识库来源，显示具体名称
  if (sourceType === 'QUESTION_BANK' && question.questionBankName) {
    name += ` (${question.questionBankName})`
  } else if (sourceType === 'KNOWLEDGE_BASE' && question.knowledgeBaseName) {
    name += ` (${question.knowledgeBaseName})`
  }
...
// 题目操作方法
const viewQuestion = (id: number | string) => {
  router.push(`/questions/${id}`)
}

const editQuestion = (id: number | string) => {
...
const confirmAIOrganizedQuestion = async (id: number) => {
  try {
    await questionApi.confirmAIOrganizedQuestion(id)
    ElMessage.success('已确认AI整理题目')
    loadQuestions()
  } catch (error) {
    console.error('确认AI整理题目失败:', error)
...
const generateQuestions = () => {
  router.push('/questions/ai-generate')
}
